...
module "vm" {
  ...
  # VM identity
  vm_name       = each.key
  computer_name = try(each.value.computer_name, substr(each.key, 0, 15))
  ...
  # Domain join
  windows_domain          = try(each.value.windows_domain, var.windows_domain)
  windows_domain_user     = try(each.value.windows_domain_user, var.windows_domain_user)
  windows_domain_password = try(each.value.windows_domain_password, var.windows_domain_password)
  windows_domain_ou       = try(each.value.windows_domain_ou, var.windows_domain_ou)
  ...
  vbs_enabled                 = try(each.value.vbs_enabled, var.vbs_enabled)
  efi_secure_boot_enabled     = try(each.value.efi_secure_boot_enabled, var.efi_secure_boot_enabled)
